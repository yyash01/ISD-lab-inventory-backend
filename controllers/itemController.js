...
exports.FSN1 = (req, res) => {
  var query1 = {
    date_purchased: { $lt: new Date(new Date() - 43800 * 60 * 60 * 1000) },
  }; // we have to take the item_id of the item which we want to add into inventory.
...
exports.FSN2 = (req, res) => {
  var query2 = {
    date_purchased: { $lt: new Date(new Date() - 2 * 43800 * 60 * 60 * 1000) },
  }; // we have to take the item_id of the item which we want to add into inventory.
...
exports.addItem = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ status: "Item Not Added Successfully", item: false, error: err });
  }
};
...
exports.toInventory = (req, res) => {
  ...
  Item.findOneAndUpdate(
    { _id: item_id },
    { $inc: { item_count: 1, issued_count: -1 } },
    function (err, doc) {
      if (err) {
        res.status(404).json({ error: err });
      } else {
